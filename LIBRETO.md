...
*[Cambia a Colab o VS Code. Ten la ventana YA ABIERTA en otra pestaña, no la busques en vivo.]*

> Tengo cinco mil ciento cincuenta registros de ventas, con duplicados y con precios faltantes metidos a propósito. Y una pregunta de negocio: ¿cuáles son los ingresos por ciudad y categoría?

---

## SLIDES 14-16 · La demo — `11:00 → 13:30`
...
**Paso 2 — leer:**
> Declaro el esquema a mano en vez de usar `inferSchema`. ¿Por qué? Porque inferir obliga a Spark a leer el archivo **dos veces**: una para adivinar los tipos y otra para cargarlo. Con cinco mil filas no se nota, con quinientos gigas estás pagando el doble de lectura.
...
**Paso 3 — limpiar:**
> Cuento los nulos, quito duplicados, imputo los precios faltantes con el promedio. Pasamos de cinco mil ciento cincuenta filas a cinco mil, y de ciento sesenta y cinco nulos a cero.

**Paso 4 — transformar:**
> Creo la columna `total`, saco el mes de la fecha, y segmento las ventas con `when` y `otherwise`, que es el `CASE WHEN` de SQL.

**Paso 5 — unir:**
> Uno la tabla de ventas con la de tiendas. La de hechos con la dimensión. Esto es, literalmente, un esquema estrella. Y ojo: aquí hay un shuffle.

**Paso 6 — la respuesta:**
> `groupBy` por ciudad y categoría, y agrego ingresos, número de ventas y ticket promedio.

*[Muestra la salida.]*
> Ahí está la respuesta al negocio.
...
## SLIDE 17-18 · Los cuadros de referencia — `13:30 → 14:00`
...
plan = (ventas
        ...
        .groupBy("categoria")
        .agg(F.sum("precio_unitario").alias("total")))
...
```
FileScan csv [categoria, cantidad, precio_unitario]
```

Y ahí dices:

> Fíjense en esto. El archivo tiene **seis** columnas, pero el plan dice que Spark solo va a leer **tres**: las únicas que mi consulta necesita. Yo nunca le dije que hiciera eso. El optimizador se dio cuenta solo, porque vio la receta completa antes de cocinar. **Eso** es para lo que sirve la evaluación perezosa.
...
## 2. Di en voz alta lo que suena mal
...
## 3. Prepara las tres preguntas difíciles
...
## 4. Detalles de ejecución que suman más de lo que parecen
...
4. Los pasos 4 y 5 de la demo — vas directo del limpiado a la agregación

**Lo que NO se recorta nunca:** transformaciones vs acciones (slide 7), la demo corriendo, y el `explain()`. Ese es tu núcleo.
